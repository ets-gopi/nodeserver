    ...
    validate(data){
        ...
        for(const key in this.schema){
            ...
            //console.log(value,key,rules.type);
             //Handle the Objects
             if(rules.required && typeof rules.type ==="object" && !Array.isArray(rules.type)){
            ...
            if(rules.required && Array.isArray(rules.type)){
                ...
                if(!Array.isArray(value)){
                    ...
                    if(valueType && typeof valueType==="string"){
                        const invalidItems=value.filter((item,ind)=>typeof item !== valueType);
            ...
            // check the minLength
            if(rules.minLength && rules.minLength > 0 && (typeof value==="string" && value.length > 0 && value.length < rules.minLength) ){
                errors[key]=`${key} value must be atleast ${rules.minLength} characters long.`
            }
            ...
            if(checkIn && checkOut){
                ...
                if (checkInDate < currentDate) {
                    errors['checkIn'] = `Check-in date must be today or after the current date`;
                    console.log('Check-in date:', checkInDate, 'Current date:', currentDate);
                    continue;
                }
