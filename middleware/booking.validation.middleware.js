...
} = require("../utils/schemaforValidation");
...
const orderDataValidation = async (req, res, next) => {
  //console.log(req.body);
  const { id } = req.payload;
  try {
    if (Object.keys(req.body).length === 0) {
    ...
    const bookingObj = new Validator(createOrderIdDataSchema);
    const { data, errors } = bookingObj.validate({
      ...req.body,
      propertyId: req.params.propertyId,
    });
...
module.exports = {bookingValidation,orderDataValidation};
